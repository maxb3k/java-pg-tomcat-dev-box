...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision :shell, path: "setup_utils.sh"
  config.vm.provision :shell, path: "setup_java.sh"
  config.vm.provision :shell, path: "setup_tomcat.sh"
  config.vm.provision :shell, path: "setup_pgsql.sh"
  ...
  config.vm.network "forwarded_port", guest: 9443, host: 9443 #wso2
  ...
  #config.vm.synced_folder "./etc", "/etc", create:true
